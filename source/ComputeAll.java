...
public class ComputeAll{
    ...
    public void computeCMTC(){
        ...
        reverse.execute();
        setWhite(vertexArray);
        double cmtc, forwardCost, reverseCost;
        ...
        double[][] outputArray = new double[numRows][numCols];
        for(int i =0; i<numRows; i++){
            for(int j =0; j<numCols; j++){
                forwardCost = forward.getShortestDistance(vertexArray[i][j]);
                reverseCost = reverse.getShortestDistance(vertexArray[i][j]);
                ...
                if(checkValue<threshold){
                    outputArray[i][j] = checkValue/maxCost;//scale so value will be between 0 and 1
                }
                else{
                    outputArray[i][j] = -9999;//mask out cells with value larger then treshold
                }
            }
        }
        this.CMTC = outputArray;
    }//end computeCMTC
